...
import { BUNDLED_OH_MY_BRAIN_EXTENSION_ID, getBundledOhMyBrainExtensionFactory } from "./defaults/gjc-oh-my-brain";
...
export async function createAgentSession(options: CreateAgentSessionOptions = {}): Promise<CreateAgentSessionResult> {
	...
	try {
		...
		if (!extensionsResult.extensions.some(extension => extension.path === BUNDLED_OH_MY_BRAIN_EXTENSION_ID)) {
			const bundledOhMyBrainExtension = await loadExtensionFromFactory(
				getBundledOhMyBrainExtensionFactory(),
				cwd,
				eventBus,
				extensionsResult.runtime,
				BUNDLED_OH_MY_BRAIN_EXTENSION_ID,
			);
			extensionsResult.extensions.push(bundledOhMyBrainExtension);
		}
